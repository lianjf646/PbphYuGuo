...
import com.pbph.yuguo.adapter.EventManjianViewHolder;
...
public class EventManjianActvity extends BaseActivity {

    TextView tv_event_mianjian1, tv_event_mianjian2;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_event_manjian);
    ...
    private void initView() {
        ...
        adapter = new DataAdapter(mContext, mLvGroupPurchaseGoodsList, EventManjianViewHolder.class);
        adapter.onItemViewClickListener = (rid, holder, objs) -> {
            ...
            addShoppingCart(YuGuoApplication.userInfo.getCustomerId(), vo.getStoreGoodsId(), storeId, 1, vo.getStoreGoodsInfoId(), acticeId, 1);
    ...
    private void getActivityGoodsInfoList() {
        ...
        HttpAction.getInstance().getActivityGoodsInfoList(resquest).subscribe(new BaseObserver<>(mContext, (response -> {
            ...
            setEventInfo(act.getIsActivityOk(), act.getPrice(), act.getCount(), act.getSatisfactionNum(), act.getStartTime(), act.getEndTime());
    ...
    private void addShoppingCart(int customerId,
        ...
        HttpAction.getInstance().addShoppingCart(new AddShoppingCartResquest(customerId, storeGoodsId, storeId, goodsNum, storeGoodsInfoId, activeId, activeType)).subscribe(new BaseObserver<>(mContext, (response -> {
            ...
            setEventInfo(act.getIsActivityOk(), act.getFullXmoney(), act.getYNumber(), act.getSatisfactionNum(), null, null);
    ...
    private void setEventInfo(int isActivityOk, int price1, int count, int SatisfactionNum, String startTime, String endTime) {
        String moneyAll = MoneyHelper.getInstance4Fen(price1).change2Yuan().getStringDelZero();
        if (isActivityOk == 0) {
            //未满足活动
            tv_event_mianjian1.setText(moneyAll);
            tv_event_mianjian1.append("元");
            tv_event_mianjian1.append(String.valueOf(count));
            tv_event_mianjian1.append("件，再购");
            tv_event_mianjian1.append(String.valueOf(SatisfactionNum));
            tv_event_mianjian1.append("件即享");
        } else {
            //满足活动
            tv_event_mianjian1.setText(moneyAll);
            tv_event_mianjian1.append("元");
            tv_event_mianjian1.append(String.valueOf(count));
            tv_event_mianjian1.append("件");
        }
